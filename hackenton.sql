...
/*!50003 DROP PROCEDURE IF EXISTS `sp_tps_delete` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_tps_delete`(
    IN p_actor_idUser INT,
    IN p_idTps BIGINT -- ID TPS yang akan dihapus
)
BEGIN
    DECLARE v_idSubDistrict BIGINT;
    
    -- Ambil lokasi (kelurahan) dari TPS yang akan dihapus untuk validasi
    ...
    -- Validasi Otoritas Aktor berdasarkan lokasi TPS
    CALL sp_validateUser(p_actor_idUser, v_idSubDistrict, 'sub_district');
    
    -- Jika validasi berhasil (tidak ada error), lakukan DELETE
    DELETE FROM tpsdata WHERE idTps = p_idTps;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_users_create`(
    ...
    -- Data untuk penugasan di tabel '...usersdetail'
    IN p_assignmentId BIGINT -- ID dari provinsi/kota/kecamatan/dll.
)
BEGIN
    ...
    BEGIN
        ROLLBACK;
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Gagal membuat pengguna. Terjadi kesalahan database.';
    END;

    -- Langkah 1: Validasi apakah Aktor punya wewenang di wilayah penugasan user baru.
    -- Kita gunakan role sebagai target levelnya.
    CALL sp_validateUser(p_actor_idUser, p_assignmentId, p_role);

    -- Langkah 2: Mulai Transaction
    START TRANSACTION;

    -- Langkah 3: Insert ke tabel 'users' dengan password yang sudah di-hash
    ...
    -- Langkah 4: Insert ke tabel detail yang sesuai berdasarkan role
    IF p_role = 'province' THEN
        INSERT INTO provinceusersdetail (idUser, name, idProvince) VALUES (v_new_idUser, p_name, p_assignmentId);
    ELSEIF p_role = 'city' THEN
        INSERT INTO cityusersdetail (idUser, name, idCity) VALUES (v_new_idUser, p_name, p_assignmentId);
    ELSEIF p_role = 'district' THEN
        INSERT INTO districtusersdetail (idUser, name, idDistrict) VALUES (v_new_idUser, p_name, p_assignmentId);
    ELSEIF p_role = 'sub_district' THEN
        INSERT INTO subdistrictusersdetail (idUser, name, idSubDistrict) VALUES (v_new_idUser, p_name, p_assignmentId);
    ELSEIF p_role = 'adminTps' THEN
        INSERT INTO admintpsuserdetail (idUser, name, idTps) VALUES (v_new_idUser, p_name, p_assignmentId);
    ELSEIF p_role = 'officerTps' THEN
        INSERT INTO officertpsuserdetail (idUser, name, idTps) VALUES (v_new_idUser, p_name, p_assignmentId);
    ELSEIF p_role = 'national' THEN
        INSERT INTO nationalusersdetail (idUser, name) VALUES (v_new_idUser, p_name);
    END IF;
...
BEGIN
    ...
    IF p_actor_idUser = p_target_idUser THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Anda tidak dapat menghapus akun Anda sendiri.';
    END IF;

    -- Langkah 1: Dapatkan lokasi/penugasan dari user target untuk validasi
    SELECT 
        COALESCE(idTps, idSubDistrict, idDistrict, idCity, idProvince),
        COALESCE(
            CASE WHEN idTps IS NOT NULL THEN 'tps' END,
            CASE WHEN idSubDistrict IS NOT NULL THEN 'sub_district' END,
            CASE WHEN idDistrict IS NOT NULL THEN 'district' END,
            CASE WHEN idCity IS NOT NULL THEN 'city' END,
            CASE WHEN idProvince IS NOT NULL THEN 'province' END,
            'national'
        )
    ...
    -- Langkah 2: Validasi apakah aktor punya wewenang
    CALL sp_validateUser(p_actor_idUser, v_target_assignmentId, v_target_level);

    -- Langkah 3: Hapus user. ON DELETE CASCADE akan membersihkan data di tabel lain.
    DELETE FROM users WHERE idUser = p_target_idUser;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_users_update`(
    ...
    -- Data baru (buat NULL jika tidak ingin diubah)
    IN p_new_email VARCHAR(100),
    IN p_new_name VARCHAR(255),
    IN p_new_password VARCHAR(255) -- Kirim password baru jika ingin direset
)
BEGIN
    DECLARE v_target_level VARCHAR(50);
    DECLARE v_target_assignmentId BIGINT;
    ...
    -- Langkah 1: Dapatkan lokasi/penugasan dari user target untuk validasi
    -- Kita gunakan view `vw_admin_users` untuk mempermudah
    SELECT 
        COALESCE(idTps, idSubDistrict, idDistrict, idCity, idProvince),
        COALESCE(
            CASE WHEN idTps IS NOT NULL THEN 'tps' END,
    ...
    -- Juga update nama di tabel detailnya
    -- (Ini bisa diperluas dengan IF/ELSEIF seperti di Create)
    UPDATE provinceusersdetail SET name = COALESCE(p_new_name, name) WHERE idUser = p_target_idUser;
    UPDATE cityusersdetail SET name = COALESCE(p_new_name, name) WHERE idUser = p_target_idUser;
    UPDATE districtusersdetail SET name = COALESCE(p_new_name, name) WHERE idUser = p_target_idUser;
    UPDATE subdistrictusersdetail SET name = COALESCE(p_new_name, name) WHERE idUser = p_target_idUser;
    UPDATE admintpsuserdetail SET name = COALESCE(p_new_name, name) WHERE idUser = p_target_idUser;
    UPDATE officertpsuserdetail SET name = COALESCE(p_new_name, name) WHERE idUser = p_target_idUser;
    UPDATE nationalusersdetail SET name = COALESCE(p_new_name, name) WHERE idUser = p_target_idUser;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_validateUser`(
    IN p_idUser INT,
    IN p_idTargetData BIGINT,
    IN p_targetLevel VARCHAR(50)
)
BEGIN
    DECLARE v_userRole VARCHAR(50);
    DECLARE v_authorized INT DEFAULT 0;

    -- Langkah 1: Cek apakah pengguna adalah Super Admin ('national'). 
    -- Jika ya, langsung berikan akses penuh dan hentikan pengecekan lebih lanjut.
    SELECT role INTO v_userRole FROM users WHERE idUser = p_idUser;

    IF v_userRole = 'national' THEN
        SET v_authorized = 1;
    ELSE
        -- Langkah 2: Lakukan validasi berjenjang berdasarkan level data ('p_targetLevel') yang ingin diakses.
        IF p_targetLevel = 'province' THEN
            -- Data provinsi hanya bisa diakses oleh pengguna yang ditugaskan langsung ke provinsi tersebut.
            SELECT 1 INTO v_authorized FROM provinceusersdetail WHERE idUser = p_idUser AND idProvince = p_idTargetData LIMIT 1;

        ELSEIF p_targetLevel = 'city' THEN
            -- Data kota bisa diakses oleh pengguna yang ditugaskan di kota tersebut ATAU di provinsi induknya.
            SELECT 1 INTO v_authorized
            FROM citydata cd
            WHERE cd.idCity = p_idTargetData AND (
                EXISTS (SELECT 1 FROM cityusersdetail WHERE idUser = p_idUser AND idCity = cd.idCity) OR
                EXISTS (SELECT 1 FROM provinceusersdetail WHERE idUser = p_idUser AND idProvince = cd.idProvince)
            ) LIMIT 1;

        ELSEIF p_targetLevel = 'district' THEN
            -- Data kecamatan bisa diakses oleh pengguna yang ditugaskan di kecamatan, kota, ATAU provinsi induknya.
            SELECT 1 INTO v_authorized
            FROM districtdata d
            JOIN citydata c ON d.idCity = c.idCity
            WHERE d.idDistrict = p_idTargetData AND (
                EXISTS (SELECT 1 FROM districtusersdetail WHERE idUser = p_idUser AND idDistrict = d.idDistrict) OR
                EXISTS (SELECT 1 FROM cityusersdetail WHERE idUser = p_idUser AND idCity = c.idCity) OR
                EXISTS (SELECT 1 FROM provinceusersdetail WHERE idUser = p_idUser AND idProvince = c.idProvince)
            ) LIMIT 1;

        ELSEIF p_targetLevel = 'sub_district' THEN
            -- Data kelurahan bisa diakses oleh pengguna yang ditugaskan di kelurahan, kecamatan, kota, ATAU provinsi induknya.
            -- Disesuaikan dengan ENUM 'sub_district'.
            SELECT 1 INTO v_authorized
            FROM subdistrictdata sd
            JOIN districtdata d ON sd.idDistrict = d.idDistrict
            JOIN citydata c ON d.idCity = c.idCity
            WHERE sd.idSubDistrict = p_idTargetData AND (
                EXISTS (SELECT 1 FROM subdistrictusersdetail WHERE idUser = p_idUser AND idSubDistrict = sd.idSubDistrict) OR
                EXISTS (SELECT 1 FROM districtusersdetail WHERE idUser = p_idUser AND idDistrict = d.idDistrict) OR
                EXISTS (SELECT 1 FROM cityusersdetail WHERE idUser = p_idUser AND idCity = c.idCity) OR
                EXISTS (SELECT 1 FROM provinceusersdetail WHERE idUser = p_idUser AND idProvince = c.idProvince)
            ) LIMIT 1;

        ELSEIF p_targetLevel = 'tps' THEN
            -- Data TPS bisa diakses oleh semua level di atasnya dalam satu garis wilayah administratif.
            -- Ini mencakup 'officerTps' dan 'adminTps' yang ditugaskan langsung.
            SELECT 1 INTO v_authorized
            FROM tpsdata t
            JOIN subdistrictdata sd ON t.idSubDistrict = sd.idSubDistrict
            JOIN districtdata d ON sd.idDistrict = d.idDistrict
            JOIN citydata c ON d.idCity = c.idCity
            WHERE t.idTps = p_idTargetData AND (
                EXISTS (SELECT 1 FROM officertpsuserdetail WHERE idUser = p_idUser AND idTps = t.idTps) OR
                EXISTS (SELECT 1 FROM admintpsuserdetail WHERE idUser = p_idUser AND idTps = t.idTps) OR
                EXISTS (SELECT 1 FROM subdistrictusersdetail WHERE idUser = p_idUser AND idSubDistrict = sd.idSubDistrict) OR
                EXISTS (SELECT 1 FROM districtusersdetail WHERE idUser = p_idUser AND idDistrict = d.idDistrict) OR
                EXISTS (SELECT 1 FROM cityusersdetail WHERE idUser = p_idUser AND idCity = c.idCity) OR
                EXISTS (SELECT 1 FROM provinceusersdetail WHERE idUser = p_idUser AND idProvince = c.idProvince)
    ...
    -- Langkah 3: Jika setelah semua pengecekan v_authorized masih 0 (tidak ada hak akses ditemukan),
    -- lemparkan error untuk menghentikan proses selanjutnya.
    IF v_authorized = 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Akses ditolak: Anda tidak memiliki kewenangan untuk data di wilayah ini.';
    END IF;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_voter_create`(
    ...
    IN p_gender TINYINT, -- Misal: 1 untuk Laki-laki, 2 untuk Perempuan
...
BEGIN
    ...
    -- Dapatkan ID detail petugas dari user yang melakukan aksi
    SELECT idData INTO v_officer_detail_id 
    FROM officertpsuserdetail 
    WHERE idUser = p_actor_idUser;

    IF v_officer_detail_id IS NULL THEN
        SET p_message = 'Error: Aksi ini hanya bisa dilakukan oleh Petugas TPS (officerTps).';
    ELSE
        ...
        ELSE
            -- Masukkan data pemilih baru
            INSERT INTO voterdata(nik, name, dateOfBirth, gender, idOfficer, locationPhoto)
            VALUES (p_nik, p_name, p_dateOfBirth, p_gender, v_officer_detail_id, p_locationPhoto);
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_voter_delete` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_voter_delete`(
    IN p_actor_idUser INT,
    IN p_idData INT -- ID pemilih yang akan dihapus
)
BEGIN
    DECLARE v_idTps BIGINT;
    
    -- Dapatkan ID TPS dari data pemilih yang akan dihapus
    SELECT idTps INTO v_idTps FROM vw_voter_details WHERE idData = p_idData;

    IF v_idTps IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Error: Data pemilih tidak ditemukan.';
    END IF;

    -- Validasi Otoritas Aktor
    CALL sp_validateUser(p_actor_idUser, v_idTps, 'tps');
    
    -- Lakukan DELETE
    DELETE FROM voterdata WHERE idData = p_idData;
...
BEGIN
    ...
    WHERE
        ...
        (
            ...
            (v_actor_role IN ('adminTps', 'officerTps') AND vw_voter_details.idTps = v_actor_idTps)
        )
        -- LAPISAN 2: FILTER PENCARIAN (OPSIONAL)
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_voter_update`(
    IN p_actor_idUser INT,
    IN p_idData INT, -- ID pemilih yang akan diubah
    -- Parameter baru (biarkan NULL jika tidak ingin diubah)
    IN p_nik VARCHAR(16),
    IN p_name VARCHAR(255),
    IN p_dateOfBirth DATE,
...
BEGIN
    ...
    -- Dapatkan ID TPS dari data pemilih yang akan diubah
    SELECT idTps INTO v_idTps FROM vw_voter_details WHERE idData = p_idData;
    ...
    -- LANGKAH 1: Validasi Otoritas Aktor
    CALL sp_validateUser(p_actor_idUser, v_idTps, 'tps');
    
    -- LANGKAH 2: Validasi duplikasi NIK jika NIK diubah
    IF p_nik IS NOT NULL THEN
        SELECT COUNT(*) INTO v_nik_exists 
        FROM voterdata WHERE nik = p_nik AND idData != p_idData;
        IF v_nik_exists > 0 THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Error: NIK sudah digunakan oleh pemilih lain.';
        END IF;
    END IF;

    -- LANGKAH 3: Lakukan UPDATE, gunakan COALESCE untuk menjaga nilai lama jika input NULL
    UPDATE voterdata
    SET
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vw_tps_details` AS select `t`.`idTps` AS `idTps`,`t`.`name` AS `name`,`sd`.`idSubDistrict` AS `idSubDistrict`,`sd`.`name` AS `subDistrictName`,`d`.`idDistrict` AS `idDistrict`,`d`.`name` AS `districtName`,`c`.`idCity` AS `idCity`,`c`.`name` AS `cityName`,`p`.`idProvince` AS `idProvince`,`p`.`name` AS `provinceName` from ((((`tpsdata` `t` join `subdistrictdata` `sd` on((`t`.`idSubDistrict` = `sd`.`idSubDistrict`))) join `districtdata` `d` on((`sd`.`idDistrict` = `d`.`idDistrict`))) join `citydata` `c` on((`d`.`idCity` = `c`.`idCity`))) join `provincedata` `p` on((`c`.`idProvince` = `p`.`idProvince`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vw_voter_details` AS select `v`.`idData` AS `idData`,`v`.`nik` AS `nik`,`v`.`name` AS `name`,`v`.`dateOfBirth` AS `dateOfBirth`,`v`.`gender` AS `gender`,`v`.`locationPhoto` AS `locationPhoto`,`v`.`idOfficer` AS `idOfficer`,`u`.`name` AS `officerName`,`t`.`idTps` AS `idTps`,`t`.`name` AS `tpsName`,`sd`.`idSubDistrict` AS `idSubDistrict`,`sd`.`name` AS `subDistrictName`,`d`.`idDistrict` AS `idDistrict`,`d`.`name` AS `districtName`,`c`.`idCity` AS `idCity`,`c`.`name` AS `cityName`,`p`.`idProvince` AS `idProvince`,`p`.`name` AS `provinceName` from (((((((`voterdata` `v` join `officertpsuserdetail` `otud` on((`v`.`idOfficer` = `otud`.`idData`))) join `users` `u` on((`otud`.`idUser` = `u`.`idUser`))) join `tpsdata` `t` on((`otud`.`idTps` = `t`.`idTps`))) join `subdistrictdata` `sd` on((`t`.`idSubDistrict` = `sd`.`idSubDistrict`))) join `districtdata` `d` on((`sd`.`idDistrict` = `d`.`idDistrict`))) join `citydata` `c` on((`d`.`idCity` = `c`.`idCity`))) join `provincedata` `p` on((`c`.`idProvince` = `p`.`idProvince`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2025-06-08 18:17:52
